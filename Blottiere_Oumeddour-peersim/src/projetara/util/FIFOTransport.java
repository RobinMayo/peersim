...
public class FIFOTransport implements Transport, EDProtocol {
	...
	public void send(Node src, Node dest, Object msg, int pid) {
		...
		t.send(src, dest, new FIFOMessage(src.getID(), dest.getID(), msg, numseq, protocol_id), protocol_id);
				...
				for(FIFOMessage fifomess : pendMsg){
					...
					if(fifomess.getIdSrc() ==id_sender ){
						...
						if(fifomess.getNumseq() == next){
							...
							((EDProtocol)node.getProtocol(content.getPid())).processEvent(node, content.getPid(), content); 
	...
	private static class FIFOMessage extends Message{
		...
		public FIFOMessage(long idsrc, long iddest, Object content,int numseq, int pid) {
			super(idsrc, iddest, "FIFOMessage", content, pid);
			this.numseq=numseq;
